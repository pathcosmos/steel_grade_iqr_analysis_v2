...
### 12.1 이전 일별 분석의 핵심 결론 2개가 역인과로 판명

코일 단위(±30초) 매칭 + RUN 필터 + 강종/규격 층화 분석을 통해, 이전 일별 집계 분석(MANAGEMENT_REPORT)의 핵심 결론 중 2개가 **역인과(reverse causation)**임이 실증되었다.

| 이전 결론 (일별 집계) | 코일 단위 검증 | 최종 판정 |
|----------------------|---------------|----------|
| "스탠드 토크 변동 = 최강 지표" (Pearson r=0.62, Composite 1위) | RUN 필터 시 p=0.259로 **완전 소멸**. 규격별 방향 반전(Simpson's Paradox) | **역인과 확정** — 불량→정지→토크변동 |
| "빌렛 추출 온도 +32.5°C = 위험" (Mann-Whitney p=0.026) | 코일 단위에서 p=0.877로 **완전 비유의** | **역인과 확정** — 일별 집계 허위 상관 |
| "정지 후 2시간 = 불량 확률 3배" (OR=3.0) | 불량의 50%가 TRANS/IDLE 상태에서 매칭 | **부분 유지** — TRANS 상태 자체가 핵심 |

**역인과 메커니즘**: 같은 날 불량 발생 → 라인 정지/감속 반복 → IDLE/TRANS 구간 증가 → 토크값 ~0으로 급감 → 일별 평균에서 "토크 변동 증가"로 나타남. 빌렛 온도도 정지 중 가열로 체류로 변동 → 일별 평균에 반영.

### 12.2 진짜 확정된 불량 인자 3가지

코일 단위 + RUN 필터 + confounding 제거 후에도 유의하게 남는 **진짜 시그널**:

| 순위 | 인자 | 근거 | 적용 범위 | 신뢰도 |
|------|------|------|----------|--------|
| **1** | **가열로 압력(FURNACE_PRESSURE) 저하** | RUN p=0.000021, confounding 14%만, D4-D16(p=0.0002)/D5-D10(p=0.040)/N5(p=0.014) 독립 유의 | **전 강종** | **최고** |
| **2** | **PINCHROLL_3 토크 부족** | RUN p=0.000011, PR3<10 시 OR=15.7(p=0.000001). 장력 부족→권취 불안정은 교과서적 메커니즘 | **D5-D10 한정** | 높음 (confounding 42%) |
| **3** | **TRANS/IDLE 상태 자체** | 불량률: TRANS/IDLE 4.3% vs RUN 0.77% (5.5배). 불량의 50%가 비-RUN 매칭 | **전 강종** | 높음 |

**가열로 압력이 가장 신뢰할 수 있는 이유**: confounding이 14%로 가장 적고, 3개 강종(D4/D5/N5)에서 독립적으로 유의하며, 규격별 Simpson's Paradox가 발생하지 않음. 로압 저하 → 연소 불균형 → 가열 불균일 → 압연 불안정이라는 인과 경로도 공정적으로 합리적.

**PR3 토크가 D5-D10 한정인 이유**: 전체 PR3 효과(-52%)의 42%가 규격 간 baseline 차이(confounding). D5-D10 내에서만 -51.6%(p=0.004) 유의, N5-D16(-16.7%, p=0.118), N5-D12(-1.8%, p=0.329), D4-D16(0.0%, p=0.076) 모두 비유의.

### 12.3 불량 유형별 메커니즘

| 불량 유형 | 비율 | RUN 비율 | 확정된 메커니즘 |
|----------|------|---------|---------------|
| **토션** | 65% | 53% | RUN 시: PR3 토크 부족(D5-D10) + 로압 저하. TRANS 시: 전환 구간 제어 불안정 |
| **누워감김** | 12% | 0% | 전량 TRANS/IDLE — 권취기 세팅 오류 또는 비가동 중 투입 |
| **딱지** | 6% | 100% | 저온 빌렛(829°C) + 과압연(S9 +14%) — 순수 공정 인자 |
| **돼지꼬리** | 3% | 100% | 고온 빌렛(1,150°C) + 과압연 — 순수 공정 인자 |
| **권취불량** | 8% | 20% | 80%가 TRANS/IDLE — 권취 장치 과도 상태 문제 |

### 12.4 강종별 차별화된 위험 요소

| 강종 | 핵심 유의 태그 (RUN, 코일 단위) | 우선 조치 |
|------|-------------------------------|----------|
| **D4** | FURNACE_PRESSURE(p=0.004), STAND_9(p=0.009), BILLET_TEMP(p=0.009) | 가열로 압력+온도 통합 관리 |
| **D5** | PINCHROLL_3(p<0.001), BILLET_TEMP(p=0.011), FURNACE_PRESSURE(p=0.014) | PR3 토크 경보(D10), 로압 관리 |
| **N5** | PINCHROLL_3(p=0.028), FURNACE_PRESSURE(p=0.014) | 로압 관리 (STAND_9/빌렛온도 비유의) |

### 12.5 최종 수정 권고 (우선순위순)

| 순위 | 권고 | 적용 범위 | 근거 | 이전 대비 |
|------|------|----------|------|----------|
| **1** | **가열로 압력 하한 경보 (0.48 이하 시 경보)** | **전 강종** | confounding 14%, 3개 강종 독립 유의 | **신규** (이전 Tier 3→Tier 1 승격) |
| **2** | **PR3 토크 < 10 경보** | **D5-D10 한정** | OR=15.7, 교과서적 메커니즘 | **범위 축소** (전체→D5-D10) |
| **3** | **TRANS→RUN 전환 시 품질 검사** | 전 강종 | 불량 50%가 비-RUN, 불량률 5.5배 | **유지** (근거 강화) |
| **4** | **강종별 분석 의무화** | 향후 모든 분석 | Simpson's Paradox 확인 | **신규** |
| **5** | ~~STAND_9 토크 변동성 모니터링~~ | ~~취소~~ | 역인과 확정 | **취소** |
| **6** | ~~빌렛 온도 게이트~~ | ~~취소~~ | 역인과 확정 (단, 딱지 불량용 900°C 하한은 유지) | **취소** (딱지 예외) |

### 12.6 방법론 교훈

> **일별 집계 분석의 근본적 한계가 실증되었다.**
>
> 불량 발생 → 라인 정지/감속 → 센서값 변동이라는 역인과 경로가 일별 집계에서는 "공정 변수 변동 → 불량"으로 착각되었다. 5가지 독립 분석 기법(Pearson, Granger, Mann-Whitney, RF, XGBoost)이 모두 같은 결론을 냈지만, **그 결론 자체가 역인과에 오염된 허위 신호**였다.
>
> 이는 **분석 기법의 수가 아니라 데이터의 시간 해상도가 인과 판별의 핵심**임을 보여준다.
>
> 향후 모든 공정-품질 분석은:
> 1. **코일 단위 시간 매칭** (±30초 이내)
> 2. **OPERATION_STATUS = 'RUN' 필터** 필수
> 3. **강종/규격 층화** (Simpson's Paradox 방지)
> 를 기본으로 해야 한다.
...
| 과제 | 내용 | 시기 |
|------|------|------|
| 데이터 축적 | 3~6개월 데이터로 규격별 소표본 보완 | 2026 하반기 |
| 로지스틱 회귀 | 강종/규격을 공변량으로 포함한 다변량 모델 | 데이터 축적 후 |
| 딱지/돼지꼬리 심층 | RUN 100% 불량유형의 온도-토크 임계값 정밀화 | 즉시 가능 |
| MANAGEMENT_REPORT 개정 | 역인과 확인 내용 반영, 권고 사항 전면 수정 | 즉시 |
| 실시간 대시보드 | 로압 + PR3(D5-D10) + TRANS 전환 모니터링 | 1개월 내 |

---

*최종 업데이트: 2026-03-22. 필터 검증, confounding 분석, 차트 9종(F1~F9), 종합 결론(12장) 반영.*
