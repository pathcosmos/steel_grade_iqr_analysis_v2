...
2. [기법별 분석 결과 상세](#2-기법별-분석-결과-상세)
   ...
   - 2.5 M5/M6: 미실행
   - 2.6 분석 전략 문서 반영 현황
3. [앙상블 순위 통합 (Top-20)](#3-앙상블-순위-통합-top-20)
   - 3.1 가중치
   - 3.2 ALL Top-18 앙상블 랭킹
   - 3.3 카테고리별 집계
4. [강종별 분석](#4-강종별-분석)
5. [핵심 시사점 및 권고](#5-핵심-시사점-및-권고)
6. [산출물 목록](#6-산출물-목록)
7. [최종 결론](#7-최종-결론)
...
> **왜 Q80인가 (Q75, Q90이 아닌 이유)**:
> - **Q80 (상위 20%)**: 고불량 24일 vs 정상 63일 (27.6% : 72.4%) — Mann-Whitney U 검정에 적절한 그룹 크기. 두 그룹 모두 통계적 검정력을 유지할 수 있는 최소 표본 수(≥20) 확보.
> - **Q90이면**: 고불량 ~9일 → Mann-Whitney 검정력이 크게 부족하여 실질적 차이를 탐지 못할 위험.
> - **Q75이면**: 고불량 ~22일로 Q80과 유사하지만, "상위 25%"는 고불량 기준이 느슨해져 정상일과의 프로파일 차이가 약화됨.
> - Q80은 **통계적 검정력**과 **고불량 민감도** 사이의 최적 균형점.
...
> "이 태그의 일별 통계가 불량률과 함께 움직이는가?"를 정량적으로 측정한다.

#### 왜 Spearman인가 (Pearson이 아닌 이유)

| 선택 근거 | Spearman (채택) | Pearson (미채택) |
|----------|----------------|-----------------|
| **분포 가정** | 비모수적 — 가정 불요 | 정규분포 가정 필요 |
| **이상치 강건성** | 순위 변환으로 극단값 영향 완화 | 극단 불량일 1건이 상관계수를 왜곡 |
| **비선형 포착** | 단조적 비선형 관계 감지 | 선형 관계만 측정 |
| **소량 데이터** | 87일에서 모수적 가정은 위험 | 표본 수 ≥100 이상 권장 |

> 불량률 분포는 0에 밀집하고 우측 꼬리가 긴 비정규 분포이므로, Pearson 사용 시 상관계수가 왜곡될 수 있다.

#### 뭘 확인하는가

- **ρ > 0 (양의 상관)**: 태그 값 ↑ → 불량 ↑ (예: 추출 온도 편차 증가 → 불량 증가)
- **ρ < 0 (음의 상관)**: 태그 값 ↓ → 불량 ↑ (예: 가스 유량 감소 → 불량 증가)
- **`_avg`와 높은 ρ**: 태그 **수준(level) 자체**가 문제 → 공정 설정값 조정 필요
- **`_std`/`_iqr`와 높은 ρ**: 태그 **변동성(variability)**이 문제 → 공정 안정화 필요
...
> "고불량일에 이 태그의 분포가 정상일과 **통째로** 다른가?"를 검증한다.

#### M1과의 차별점

- M1(Spearman)은 **연속적 추세**를 본다: "불량률이 1%→2%→3%로 올라갈 때 태그도 같이 변하나?"
- M2는 **이진 프로파일 차이**를 본다: "고불량일(상위 20%)의 태그 값 분포가 정상일과 통째로 다른가?"
- **M1에서 ρ=0이지만 M2에서 d≥0.8인 경우**: **임계값 효과** — 특정 수준 이상에서만 불량이 급증하는 패턴을 의미하며, 이는 **알람 임계값 설정에 직결**됨.

#### 왜 Mann-Whitney U (t-test가 아닌 이유)

| 선택 근거 | Mann-Whitney U (채택) | t-test (미채택) |
|----------|---------------------|-----------------|
| **표본 크기** | 불균형(24/63일)에 강건 | 등분산 가정 위반 위험 |
| **분포 가정** | 비정규 분포에 적합 | 정규성 가정 필요 |
| **이상치** | 순위 기반으로 강건 | 극단값에 민감 |

#### 왜 Cohen's d (p-value만으로 부족한 이유)

- **p-value** = "차이가 **있는가**" (유/무) → 이진 판단
- **Cohen's d** = "차이가 **얼마나 큰가**" (크기) → 연속적 판단
- 87건 소량 데이터에서 p-value는 비유의해도 **d≥0.5면 실질적 차이 존재** — 공정 관리에서는 통계적 유의성보다 **실무적 효과 크기**가 더 중요.

#### 효과 크기 해석 기준

| 조건 | 판정 | 조치 |
|------|------|------|
| \|d\| ≥ 0.80 + p < 0.05 | **확정 알람 태그** | 고불량일에 명확히 다른 수준/변동 → 즉시 알람 설정 |
| \|d\| ≥ 0.50 + p ≥ 0.05 | **주의 태그** | 실질 차이 있으나 데이터 부족 → 데이터 축적 후 재확인 |
...
> "다수 태그를 **동시에** 고려할 때, 불량률 예측에 가장 기여하는 태그는 무엇인가?"를 식별한다.

#### M1/M2와의 핵심 차별점

- M1/M2는 **단변량 분석**: 태그 하나씩 독립적으로 불량과의 관계를 봄.
- M3는 **다변량 분석**: 18개 태그를 동시에 고려하여, **태그 간 상호작용 효과**를 포착.
- 예: "가열로 온도가 높고 + 스탠드 속도가 낮을 때" 불량 급증 → M1은 이런 조합 효과를 감지 못하지만 M3는 트리 분기로 자연스럽게 포착.

#### 왜 LightGBM

| 선택 근거 | LightGBM (채택) | sklearn GBR (미채택) |
|----------|-----------------|---------------------|
| **속도** | 18 feature × 87 샘플에서 LOO-CV(87회 학습) 빠르게 완료 | 상대적으로 느림 |
| **범주형 처리** | 향후 강종/규격 변수 추가 시 원-핫 인코딩 불요 | 원-핫 인코딩 필요 |
| **과적합 제어** | `max_depth=4, num_leaves=16`으로 87건에 맞춰 강한 제한 | 동일 제한 가능하나 덜 유연 |

#### 왜 LOO-CV (K-Fold가 아닌 이유)

- 87건 데이터에서 5-Fold = Fold당 17건 → 검증 데이터가 너무 적어 분산이 큼.
- LOO-CV: 매번 86건 학습 + 1건 검증 → **최대 학습 데이터 활용** + **편향 최소** (소량 데이터에 최적).

#### Feature Importance 해석 주의점

- **gain 기반**: 해당 feature로 분기할 때 얻는 **손실 감소량 합계** → 예측력에 대한 직접 기여도.
- **상관 태그 주의**: 상관된 태그(예: 스탠드 속도 ACTUAL vs REFERENCE) 간 importance가 분산됨 → 카테고리별 합산으로 보완.
- **gain vs split**: split 기반은 "얼마나 자주 사용됐는지"이므로 연속값 태그에 편향 → **gain이 더 신뢰성 높음**.
...
> "고불량 극단 이벤트(상위 10%)를 설명하는 태그는 무엇인가?"를 식별한다.

#### M3와의 핵심 차별점

- **M3 = 평균(mean) 예측**: "태그 값이 이러면 불량률이 평균적으로 몇 %인가?"
- **M4 = 상위 꼬리(tail) 예측**: "태그 값이 이러면 불량률이 상위 10%까지 얼마나 올라갈 수 있는가?"
- **핵심 통찰**: 불량은 **극단 이벤트**. 평균적으로 불량률 1%인 날에도 상위 10%에서는 5% 이상 발생 가능. M4는 이 극단 구간의 기여 태그를 식별.

#### 왜 Q90 (Q50, Q75가 아닌 이유)

| 분위수 | 의미 | 채택 여부 | 근거 |
|:------:|------|:--------:|------|
| Q50 | 중앙값 | ❌ | M3(평균 회귀)와 유사한 결과 → 중복 |
| Q75 | 약한 극단 | ❌ | 일반적인 "나쁜 날" 수준 → 민감도 부족 |
| **Q90** | **10일에 1일 발생하는 고불량** | ✅ | 공정 관리 알람 임계값에 직접 대응 |
| Q95+ | 극단 | ❌ | 87건 중 상위 ~4건 → 추정 불안정 |

#### 왜 선형 Quantile Regression (LightGBM Quantile이 아닌 이유)

- **선형 계수**가 직접 해석 가능: 태그당 기여도 = 계수 크기 → 공정 엔지니어에게 설명 용이.
- LightGBM Quantile은 비선형이지만 해석이 Feature Importance에 의존 → M3와 중복.
- **표준화 후 계수 비교** → 태그 간 상대적 기여도를 직접 비교 가능.
...
#### M5 — IQR 이상치 빈도 × 고불량일 교차

> **목적**: "이 태그에 이상치가 발생한 날이 고불량일과 겹치는가?"

**M1~M4와의 핵심 차별점**:
- M1~M4는 **일별 집계 통계**(avg, std, max)를 입력으로 사용 — 하루의 "평균적 상태"를 봄.
...
**Fisher's Exact OR 해석**:
- OR = 1.0: 이상치와 불량 무관 (우연 수준)
- **OR ≥ 2.0**: 이상치 발생일에 고불량 확률 **2배** → 의미 있는 연관
- OR ≥ 5.0: 매우 강한 연관 → 최우선 알람 태그 후보

#### M6 — PELT 변점 빈도 × 불량률 상관

> **목적**: "이 태그의 공정 상태 전환(regime shift) 빈도가 불량 증가와 관련되는가?"

**M1~M5와의 핵심 차별점**:
- M1~M5는 **태그 값의 크기**(수준, 분포, 이상치)를 봄 — "얼마나 높은가/낮은가"
- M6는 **시계열의 구조적 변화(regime shift)**를 봄 — "언제 공정 상태가 전환됐는가"
- **핵심 가설**: 불량은 공정이 안정 상태에서 벗어나는 **과도 구간**(전환기)에 집중 발생. 예: 롤교환 후 재설정, 강종 변경 시 온도 전환, 설비 간헐적 이상 → 이런 "전환"의 빈도가 높은 날일수록 불량 증가.

**PELT (Pruned Exact Linear Time) 알고리즘**:
- 시계열을 복수 구간으로 분할하여, 각 구간 내 통계 특성이 최대한 균일하도록 최적 분할점(changepoint)을 찾음.
- 변점 개수를 사전에 지정할 필요 없음 (penalty 파라미터로 자동 결정).
- RBF 커널 사용: 평균 변화 + 분산 변화 + 비선형 변화를 모두 감지 (linear 커널은 평균 변화만 감지).
...
> **참조 문서**: `ref/DEFECT_DATA_UTILIZATION_STRATEGY.md` (25가지 방안 + 7가지 데이터 한계)
>
> 본 분석의 M1~M6 기법 설계는 위 전략 문서의 방안을 체계적으로 반영한 결과이다.

#### 반영 완료 — 전략 문서 → 본 분석 매핑

| 전략 ID | 전략 방안 | 대응 한계 | 본 분석 반영 | 적용 결과 |
|:-------:|----------|:--------:|------------|----------|
| **A4** | 강종 코드 매핑 | L2, L6 | Section 1.2~1.3 | ALL/N5/D5/D4/B5 5개 레이어 매핑 완료 |
| **B1** | 이상치-우선 접근 (Fisher's Exact OR) | L1, L5, L7 | M5 설계 | 설계 완료, 87일<100일로 **미실행** |
| **B2** | 불량률 회귀 (LOO-CV) | L5, L7 | **M3** LightGBM FI | GBR→LightGBM 변경, LOO-CV 87 folds |
| **C3** | 변점↔불량 상관 (PELT) | L1 | M6 설계 | 설계 완료, 87일<100일로 **미실행** |
| **C4** | 불량일 vs 정상일 프로파일 비교 | L5, L7 | **M2** Mann-Whitney + Cohen's d | Q80 기반 24/63일 분리, Bonferroni 보정 |
| **E2** | PELT 변점 탐지 (ruptures) | L1, L3 | M6 설계 | PELT RBF 커널 채택, C3과 통합 |
| **E3** | Quantile Regression (Q90) | L5, L7 | **M4** Q90 | sklearn QuantileRegressor, 표준화 계수 |

#### 한계 대응 현황

| 한계 | 심각도 | 대응 방식 | 잔존 한계 |
|------|:------:|----------|----------|
| **L1** 시간 해상도 불일치 | ★★★★★ | 일별 집계(avg/std/iqr/cv/max) + 날짜 기준 INNER JOIN | 코일 단위 추적 불가 (A1/A5 미적용) |
| **L2** 강종 코드 불일치 | ★★★★ | A4 매핑 테이블 적용 완료 | **해결** |
| **L5** 소량 데이터 | ★★★★ | LOO-CV, Bonferroni 보정, 기법별 최소 일수 기준 적용 | 87일 — M5/M6 미실행, N5 이하 ML 불가 |
| **L7** 불량률=0 다수 | ★★★ | Q80 이진 라벨 + weight>0 대체 | 회귀(M3/M4)로 클래스 불균형 회피 |

#### 미반영 — 후속 과제

| 우선도 | 전략 ID | 방안 | 미반영 사유 | 적용 가능 시점 |
|:------:|:-------:|------|-----------|:------------:|
| **높음** | C1 | 불량 전조 구간 (Lag 1~3) | 동일일(Lag 0) 집중, 유의 결과 우선 확보 | 즉시 (데이터 충분) |
| **높음** | C5 | 앙상블 건강 지수 | 6가지 이상치 결과 통합 미구현 | Phase 3 |
| 중간 | A1/A5 | 번들-코일 시간 연결 | 구현 복잡도 높음 (L1 근본 해결) | 중기 |
| 중간 | A2 | remarks 정규화 | memo_prod_result 미활용 | Phase 1 |
| 중간 | E1 | Conformal Prediction | B2 선행 완료 필요 | M3 확장 |
| 중간 | E4 | Rule Mining (연관규칙) | 이상치 결과 필요 | Phase 2+ |
| 낮음 | B3/B4/E5~E8 | MIL, 준지도, FDA, DAG 등 | A1 전제 미충족 또는 장기 | 장기 |

> **현재 커버리지**: 전략 문서 Phase 1~2 핵심 8기법 중 **5건 반영 (62.5%)**.
> 87일 데이터에서 실행 가능한 기법은 거의 모두 적용했으며, 미반영 기법은 데이터 부족(100일 미만) 또는 선행 과제(A1/A5) 미완이 주 사유.
...
> **3관점 앙상블**: M1~M2는 **통계적 관점** (개별 태그의 직접 상관), M3~M4는 **모델링 관점** (다수 태그 동시 고려), M5~M6는 **패턴 관점** (이상치·변점 기반 교차 검증). 세 관점을 앙상블하여 단일 기법의 편향을 보정한다.
>
> **원래 가중치 배분 근거**: M1/M3(0.25) > M2(0.20) > M4(0.15) > M5(0.10) > M6(0.05) — **기법 독립성**(서로 다른 정보를 제공하는 정도), **정보량**(태그별 순위 변별력), **해석 용이성**(공정 엔지니어에게 설명 가능한 정도)을 기반으로 배분.
> M5/M6 미실행에 따라 해당 가중치(0.15)를 기존 비율 유지하며 M1~M4에 재분배.
