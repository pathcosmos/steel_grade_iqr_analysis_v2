...
| 카테고리 | 태그 수 | RUN | Special-Ops | Roll-Change | Coiling | A/B 라인 | 비고 |
|----------|:------:|:---:|:-----------:|:-----------:|:-------:|:--------:|------|
...
| 10_PR_Detailed | 6 | ✅ | ✅ | ✅ | ✅ | ✅ | A/B 교대 운전, 라인별 독립 분석 |
...
모든 분석 스크립트(6종)에 PR_Detailed 태그 대상 A/B 라인 분리 필터링이 적용되어 있습니다.

#### 3.5.1 DB 컬럼과 태그명의 매핑

DB `line` 컬럼은 `'L1'`/`'L2'`가 아닌 **`'A'`/`'B'`** 값을 사용합니다:

| DB `line` 값 | 의미 | 태그명 접미사 | 예시 |
|:---:|:---:|:---:|------|
| **A** | L1 라인 (A라인) | `L1` | PR6**L1**_ACT_TORQUE |
| **B** | L2 라인 (B라인) | `L2` | PR6**L2**_ACT_TORQUE |

#### 3.5.2 필터링 로직

```
1. 태그명에서 L1/L2 추출
   PR6L1_ACT_TORQUE → 'L1' → DB line='A'
   PR7L2_ACT_TORQUE → 'L2' → DB line='B'

2. 해당 라인 데이터만 필터링
   df = df[df['line'] == 'A']   # L1 태그인 경우
   df = df[df['line'] == 'B']   # L2 태그인 경우

3. 폴백 (필터 후 데이터 < 10건)
   df = df[df[tag] != 0]        # 0값 제외 방식으로 대체
```
...
| 카테고리 | A/B 필터 | 사유 |
|----------|:--------:|------|
| Cat 01~09 (79개 공유 태그) | **미적용** | L1/L2 분기점 **이전** 설비 → 라인 구분 불필요 |
| Cat 10 PR_Detailed (6개) | **적용** | L1/L2 분기점 **이후** 설비 → 독립 분석 필수 |

#### 3.5.4 적용 스크립트

| # | 스크립트 | A/B 필터 | 비고 |
|---|---------|:--------:|------|
| 1 | `steel_grade_iqr_analysis_v2.py` | ✅ | `apply_filters()` 호출 전 라인 필터 선적용 |
| 2 | `steel_grade_adjusted_iqr_analysis.py` | ✅ | 동일 |
| 3 | `steel_grade_cusum_ewma_analysis.py` | ✅ | SQL에 `line` 컬럼 추가 + 필터 |
| 4 | `steel_grade_rolling_zscore_analysis.py` | ✅ | 동일 |
| 5 | `steel_grade_mahalanobis_analysis.py` | ✅ | 카테고리 단위 분석이므로 L1/L2 태그 그룹 분리 |
| 6 | `steel_grade_stl_analysis.py` | ✅ | SQL에 `line` 컬럼 추가 + 필터 |

> **참고**: Mahalanobis 분석은 카테고리 단위 다변량 분석이므로, PR_Detailed 카테고리 진입 시 L1 태그 그룹과 L2 태그 그룹을 분리하여 각각 독립적으로 Mahalanobis 거리를 계산합니다.
...
현재 217개 IBA 태그 중 **권취 품질 개선**에 필수적인 다음 영역의 데이터가 부족합니다:

| 영역 | 현재 태그 | DB 검증 현황 | 평가 |
|------|:---------:|:----------:|:----:|
| **PR6~PR9 확장** (L1/L2) | 6개 분석 중 | 🟢 DB **42개** 확인 (36개 미활용) | ⚠️ 뷰 확장만 필요 |
| 레잉헤드 (Laying Head) | **0개** | 🔴 미확인 | ❌ 심각 |
| 스텔모어/냉각대 (Stelmor) | **0개** | 🔴 미확인 | ❌ 심각 |
| VCC (권취기) | **0개** | 🟡 소재감지 3개만 PLC 참조 | ❌ 심각 |
| 디스트리뷰터 | **0개** | 🟡 턴수 4개만 PLC 참조 | ❌ 심각 |
| 코일 품질 실적 | **0개** | 🔴 미확인 | ❌ 심각 |
...
| 핀치롤 | DB 태그 수 | 분석 중 | 즉시 활용 가능 | L2 현황 |
|--------|:----------:|:-------:|:-------------:|:-------:|
| PR6 (L1/L2) | 14개 | 2개 (토크) | 12개 | ✅ L1+L2 |
| PR7 (L1/L2) | 14개 | 2개 (토크) | 12개 | ✅ L1+L2 |
| PR8 (**L1만**) | **7개** | 1개 (L1 토크) | 6개 | ❌ L2 미존재 |
| PR9 (**L1만**) | **7개** | 1개 (L1 토크) | 6개 | ❌ L2 미존재 |
...
아래 태그명은 HMI 매뉴얼 기반 **권장명**이며, 실제 PLC 태그명은 현장 확인이 필요합니다. DB 교차 검증 결과 대부분이 PLC 목록/DB에 미존재하여 서술형 요구사항으로 관리합니다.

> **상세 내역**: [`VCC_TAG_DATA_REQUEST.md`](VCC_TAG_DATA_REQUEST.md) v1.2에 영역별 측정 항목, 단위, 품질 영향도, 결정 체크박스가 포함되어 있습니다.

| 영역 | 확정 태그 | 추정 항목 | DB 상태 | 비고 |
|------|:---------:|:---------:|:-------:|------|
| VCC 본체 + 디스트리뷰터 | 7개 | ~52항목 | 🟡 일부 | 소재감지 3개 + 턴수 4개 PLC 참조 |
| 코일 형상/중량/계량 | - | ~15항목 | 🔴 | 전체 미확인 |
| 레잉헤드 | - | ~4항목 | 🔴 | 압연 HMI 영역 |
| 라인 속도 | 2개 | ~2항목 | 🟢 일부 | GENER_LINE/MILL_SPEED DB 확인 |
| 스텔모어/냉각대 | - | ~40항목 | 🔴 | VCC HMI 외부 |
| 루퍼 + Water Box | 5개 | ~46항목 | 🟡/🟢 일부 | 소재감지만 확인 |
| PR8L2/PR9L2 | - | **14개** | 🔴 | DB에 컬럼 미존재, 신규 수집 |
| **합계** | **14개** | **~173항목** | | |

### 14.4 태그 수집 우선순위 Phase

| Phase | 영역 | 확정 태그 | 추정 항목 | 예상 효과 |
|:-----:|------|:---------:|:---------:|-----------|
| **즉시** | PR6~PR9 기존 DB 태그 뷰 확장 | **36개** | - | 핀치롤 종합 진단 (수집 불필요) |
| **1** | VCC 본체 + 디스트리뷰터 + 코일 형상 | 7개 | ~56항목 | 코일 형상 분석 핵심 데이터 확보 |
| **2** | 레잉헤드 + 코일 품질 실적 + 라인 속도 | 2개 | ~16항목 | 냉각 시작점 + 품질 지표 정량화 |
| **3** | 스텔모어 + 루퍼 + Water Box + PR8/9 L2 | 1개 | ~100항목 | 정밀 분석 역량 강화 (전 공정 커버) |

> **선행 조건**: PLC24/25/52/101/102의 태그 목록을 `iba_tag_usage_check`에 임포트하면 VCC 운전 태그명을 확정할 수 있습니다.

### 14.5 추가 태그 수집 시 예상 분석 가능 항목
...
| 문서 | 위치 | 설명 |
|------|------|------|
| VCC 태그 수집 요청서 | [`VCC_TAG_DATA_REQUEST.md`](VCC_TAG_DATA_REQUEST.md) | VCC/권취부 태그 수집 요청 (v1.2, DB 검증 완료) |
